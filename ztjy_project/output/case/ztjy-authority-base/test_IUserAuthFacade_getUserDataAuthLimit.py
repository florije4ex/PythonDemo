...
class Test_IUserAuthFacade_getUserDataAuthLimit(object):
    def setup_class(self):
        self._iuserauthfacade = IUserAuthFacade()

    @pytest.mark.parametrize('userId, dataCode, userTypeEnum, officeId', pairs_datas())
    def test_IUserAuthFacade_getUserDataAuthLimit(self, userId, dataCode, userTypeEnum, officeId):
        params = self._iuserauthfacade.generator_params(userId=userId, dataCode=dataCode, userTypeEnum=userTypeEnum, officeId=officeId)
        try:
            result = self._iuserauthfacade.getUserDataAuthLimit(params=params)
