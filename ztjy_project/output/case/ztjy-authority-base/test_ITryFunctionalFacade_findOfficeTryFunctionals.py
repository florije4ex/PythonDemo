...
class Test_ITryFunctionalFacade_findOfficeTryFunctionals(object):
    def setup_class(self):
        self._itryfunctionalfacade = ITryFunctionalFacade()

    @pytest.mark.parametrize('officeId, functionalCode, systemTypeEnum, page', pairs_datas())
    def test_ITryFunctionalFacade_findOfficeTryFunctionals(self, officeId, functionalCode, systemTypeEnum, page):
        params = self._itryfunctionalfacade.generator_params(officeId=officeId, functionalCode=functionalCode, systemTypeEnum=systemTypeEnum, page=page)
        try:
            result = self._itryfunctionalfacade.findOfficeTryFunctionals(params=params)
