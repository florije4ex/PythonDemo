...
class Test_IUserAuthFacade_findNotAuthFunctionalInOffice(object):
    def setup_class(self):
        self._iuserauthfacade = IUserAuthFacade()

    @pytest.mark.parametrize('officeId, userId, userTypeEnum', pairs_datas())
    def test_IUserAuthFacade_findNotAuthFunctionalInOffice(self, officeId, userId, userTypeEnum):
        params = self._iuserauthfacade.generator_params(officeId=officeId, userId=userId, userTypeEnum=userTypeEnum)
        try:
            result = self._iuserauthfacade.findNotAuthFunctionalInOffice(params=params)
