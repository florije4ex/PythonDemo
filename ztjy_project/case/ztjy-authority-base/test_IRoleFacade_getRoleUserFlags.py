...
class Test_IRoleFacade_getRoleUserFlags(object):
    def setup_class(self):
        self._irolefacade = IRoleFacade()

    @pytest.mark.parametrize('userIds, userType, roleId, officeId', pairs_datas())
    def test_IRoleFacade_getRoleUserFlags(self, userIds, userType, roleId, officeId):
        params = self._irolefacade.generator_params(userIds=userIds, userType=userType, roleId=roleId, officeId=officeId)
        try:
            result = self._irolefacade.getRoleUserFlags(params=params)
