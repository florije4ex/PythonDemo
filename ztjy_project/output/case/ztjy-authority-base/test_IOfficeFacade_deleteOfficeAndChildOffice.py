...
class Test_IOfficeFacade_deleteOfficeAndChildOffice(object):
    def setup_class(self):
        self._iofficefacade = IOfficeFacade()

    @pytest.mark.parametrize('officeId, userId, userCenterEnum', pairs_datas())
    def test_IOfficeFacade_deleteOfficeAndChildOffice(self, officeId, userId, userCenterEnum):
        params = self._iofficefacade.generator_params(officeId=officeId, userId=userId, userCenterEnum=userCenterEnum)
        try:
            result = self._iofficefacade.deleteOfficeAndChildOffice(params=params)
