...
class Test_IFunctionalFacade_getOfficeFunctionalPackages(object):
    def setup_class(self):
        self._ifunctionalfacade = IFunctionalFacade()

    @pytest.mark.parametrize('['officeId']', '')
    def test_IFunctionalFacade_getOfficeFunctionalPackages(self, officeId):
        params = self._ifunctionalfacade.generator_params(officeId=officeId)
        try:
            result = self._ifunctionalfacade.getOfficeFunctionalPackages(params=params)
