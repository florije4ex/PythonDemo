...
class Test_IFunctionalFacade_getOfficeCloseFunctionals(object):
    def setup_class(self):
        self._ifunctionalfacade = IFunctionalFacade()

    @pytest.mark.parametrize('['officeId']', '')
    def test_IFunctionalFacade_getOfficeCloseFunctionals(self, officeId):
        params = self._ifunctionalfacade.generator_params(officeId=officeId)
        try:
            result = self._ifunctionalfacade.getOfficeCloseFunctionals(params=params)
