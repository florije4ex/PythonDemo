...
class Test_IOfficeFacade_changeParentOffice(object):
    def setup_class(self):
        self._iofficefacade = IOfficeFacade()

    @pytest.mark.parametrize('officeId, parentOfficeId, userId, userCenterEnum', pairs_datas())
    def test_IOfficeFacade_changeParentOffice(self, officeId, parentOfficeId, userId, userCenterEnum):
        params = self._iofficefacade.generator_params(officeId=officeId, parentOfficeId=parentOfficeId, userId=userId, userCenterEnum=userCenterEnum)
        try:
            result = self._iofficefacade.changeParentOffice(params=params)
