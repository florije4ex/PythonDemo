...
def pairs_datas():
    ...
    """
    param_data = [
        ['ignore', None, '', 'FUNCTIONALID', 'FU' * 11],  # FUNCTIONALID
...
class Test_IFunctionalFacade_delFunctional(object):
    def setup_class(self):
        self._ifunctionalfacade = IFunctionalFacade()

    @pytest.mark.parametrize('functionalId, force, operateUserId, userCenterEnum', pairs_datas())
    def test_IFunctionalFacade_delFunctional(self, functionalId, force, operateUserId, userCenterEnum):
        params = self._ifunctionalfacade.generator_params(functionalId=functionalId, force=force, operateUserId=operateUserId, userCenterEnum=userCenterEnum)
        try:
            result = self._ifunctionalfacade.delFunctional(params=params)
