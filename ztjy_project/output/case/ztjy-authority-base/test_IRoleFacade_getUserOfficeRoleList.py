...
class Test_IRoleFacade_getUserOfficeRoleList(object):
    def setup_class(self):
        self._irolefacade = IRoleFacade()

    @pytest.mark.parametrize('userId, userType, officeId', pairs_datas())
    def test_IRoleFacade_getUserOfficeRoleList(self, userId, userType, officeId):
        params = self._irolefacade.generator_params(userId=userId, userType=userType, officeId=officeId)
        try:
            result = self._irolefacade.getUserOfficeRoleList(params=params)
