...
class Test_IFunctionalFacade_getFunctionalPackage(object):
    def setup_class(self):
        self._ifunctionalfacade = IFunctionalFacade()

    @pytest.mark.parametrize('['functionalPackageId']', '')
    def test_IFunctionalFacade_getFunctionalPackage(self, functionalPackageId):
        params = self._ifunctionalfacade.generator_params(functionalPackageId=functionalPackageId)
        try:
            result = self._ifunctionalfacade.getFunctionalPackage(params=params)
