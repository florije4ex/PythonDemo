...
def pairs_datas():
    ...
    """
    param_data = [
        ['ignore', None, '', 'OFFICEDTO', 'OF' * 11],  # OFFICEDTO
...
class Test_IOfficeFacade_syncOfficeInfo(object):
    def setup_class(self):
        self._iofficefacade = IOfficeFacade()

    @pytest.mark.parametrize('officeDTO, userId, userCenterEnum', pairs_datas())
    def test_IOfficeFacade_syncOfficeInfo(self, officeDTO, userId, userCenterEnum):
        params = self._iofficefacade.generator_params(officeDTO=officeDTO, userId=userId, userCenterEnum=userCenterEnum)
        try:
            result = self._iofficefacade.syncOfficeInfo(params=params)
