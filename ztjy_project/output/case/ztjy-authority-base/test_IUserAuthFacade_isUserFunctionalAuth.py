...
class Test_IUserAuthFacade_isUserFunctionalAuth(object):
    def setup_class(self):
        self._iuserauthfacade = IUserAuthFacade()

    @pytest.mark.parametrize('userId, functionalCode, officeId, userTypeEnum, users', pairs_datas())
    def test_IUserAuthFacade_isUserFunctionalAuth(self, userId, functionalCode, officeId, userTypeEnum, users):
        params = self._iuserauthfacade.generator_params(userId=userId, functionalCode=functionalCode, officeId=officeId, userTypeEnum=userTypeEnum, users=users)
        try:
            result = self._iuserauthfacade.isUserFunctionalAuth(params=params)
