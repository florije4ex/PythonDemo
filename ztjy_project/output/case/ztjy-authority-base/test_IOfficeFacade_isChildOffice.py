...
class Test_IOfficeFacade_isChildOffice(object):
    def setup_class(self):
        self._iofficefacade = IOfficeFacade()

    @pytest.mark.parametrize('officeId, parentOfficeId', pairs_datas())
    def test_IOfficeFacade_isChildOffice(self, officeId, parentOfficeId):
        params = self._iofficefacade.generator_params(officeId=officeId, parentOfficeId=parentOfficeId)
        try:
            result = self._iofficefacade.isChildOffice(params=params)
