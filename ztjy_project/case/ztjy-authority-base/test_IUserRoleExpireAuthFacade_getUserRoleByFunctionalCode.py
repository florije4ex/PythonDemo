...
class Test_IUserRoleExpireAuthFacade_getUserRoleByFunctionalCode(object):
    def setup_class(self):
        self._iuserroleexpireauthfacade = IUserRoleExpireAuthFacade()

    @pytest.mark.parametrize('functionalCode, userId, officeId, userTypeEnum', pairs_datas())
    def test_IUserRoleExpireAuthFacade_getUserRoleByFunctionalCode(self, functionalCode, userId, officeId, userTypeEnum):
        params = self._iuserroleexpireauthfacade.generator_params(functionalCode=functionalCode, userId=userId, officeId=officeId, userTypeEnum=userTypeEnum)
        try:
            result = self._iuserroleexpireauthfacade.getUserRoleByFunctionalCode(params=params)
