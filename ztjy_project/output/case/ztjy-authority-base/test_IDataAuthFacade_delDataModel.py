...
class Test_IDataAuthFacade_delDataModel(object):
    def setup_class(self):
        self._idataauthfacade = IDataAuthFacade()

    @pytest.mark.parametrize('dataModelId, userId, userCenterEnum', pairs_datas())
    def test_IDataAuthFacade_delDataModel(self, dataModelId, userId, userCenterEnum):
        params = self._idataauthfacade.generator_params(dataModelId=dataModelId, userId=userId, userCenterEnum=userCenterEnum)
        try:
            result = self._idataauthfacade.delDataModel(params=params)
