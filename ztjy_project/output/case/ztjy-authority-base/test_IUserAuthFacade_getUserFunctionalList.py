...
def pairs_datas():
    ...
    param_data = [
        ...
        ['ignore', None, '', 'USERS', 'US' * 11],  # USERS
    ]
    return AllPairs(param_data)


@allure.description('负责人：XXXXXXX')
class Test_IUserAuthFacade_getUserFunctionalList(object):
    def setup_class(self):
        self._iuserauthfacade = IUserAuthFacade()

    @pytest.mark.parametrize('userId, officeId, userTypeEnum, systemTypeEnum, users', pairs_datas())
    def test_IUserAuthFacade_getUserFunctionalList(self, userId, officeId, userTypeEnum, systemTypeEnum, users):
        params = self._iuserauthfacade.generator_params(userId=userId, officeId=officeId, userTypeEnum=userTypeEnum, systemTypeEnum=systemTypeEnum, users=users)
        try:
            result = self._iuserauthfacade.getUserFunctionalList(params=params)
