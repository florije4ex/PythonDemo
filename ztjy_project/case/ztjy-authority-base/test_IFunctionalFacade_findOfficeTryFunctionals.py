...
class Test_IFunctionalFacade_findOfficeTryFunctionals(object):
    def setup_class(self):
        self._ifunctionalfacade = IFunctionalFacade()

    @pytest.mark.parametrize('officeId, systemTypeEnum', pairs_datas())
    def test_IFunctionalFacade_findOfficeTryFunctionals(self, officeId, systemTypeEnum):
        params = self._ifunctionalfacade.generator_params(officeId=officeId, systemTypeEnum=systemTypeEnum)
        try:
            result = self._ifunctionalfacade.findOfficeTryFunctionals(params=params)
