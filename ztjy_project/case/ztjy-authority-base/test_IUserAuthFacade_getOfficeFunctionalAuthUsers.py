...
class Test_IUserAuthFacade_getOfficeFunctionalAuthUsers(object):
    def setup_class(self):
        self._iuserauthfacade = IUserAuthFacade()

    @pytest.mark.parametrize('functionalCode, officeId', pairs_datas())
    def test_IUserAuthFacade_getOfficeFunctionalAuthUsers(self, functionalCode, officeId):
        params = self._iuserauthfacade.generator_params(functionalCode=functionalCode, officeId=officeId)
        try:
            result = self._iuserauthfacade.getOfficeFunctionalAuthUsers(params=params)
