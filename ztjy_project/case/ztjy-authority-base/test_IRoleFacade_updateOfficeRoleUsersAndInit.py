...
def pairs_datas():
    ...
    param_data = [
        ...
        ['ignore', None, '', 'INITROLES', 'IN' * 11],  # INITROLES
...
class Test_IRoleFacade_updateOfficeRoleUsersAndInit(object):
    def setup_class(self):
        self._irolefacade = IRoleFacade()

    @pytest.mark.parametrize('userIds, userType, officeId, roleId, initRoles, operateUserId, systemType', pairs_datas())
    def test_IRoleFacade_updateOfficeRoleUsersAndInit(self, userIds, userType, officeId, roleId, initRoles, operateUserId, systemType):
        params = self._irolefacade.generator_params(userIds=userIds, userType=userType, officeId=officeId, roleId=roleId, initRoles=initRoles, operateUserId=operateUserId, systemType=systemType)
        try:
            result = self._irolefacade.updateOfficeRoleUsersAndInit(params=params)
