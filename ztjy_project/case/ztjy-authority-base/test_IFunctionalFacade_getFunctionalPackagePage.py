...
class Test_IFunctionalFacade_getFunctionalPackagePage(object):
    def setup_class(self):
        self._ifunctionalfacade = IFunctionalFacade()

    @pytest.mark.parametrize('pageNo, pageSize', pairs_datas())
    def test_IFunctionalFacade_getFunctionalPackagePage(self, pageNo, pageSize):
        params = self._ifunctionalfacade.generator_params(pageNo=pageNo, pageSize=pageSize)
        try:
            result = self._ifunctionalfacade.getFunctionalPackagePage(params=params)
