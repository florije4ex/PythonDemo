...
class Test_IDataAuthFacade_getDataAuthGroupByCreateOffice(object):
    def setup_class(self):
        self._idataauthfacade = IDataAuthFacade()

    @pytest.mark.parametrize('['officeId']', '')
    def test_IDataAuthFacade_getDataAuthGroupByCreateOffice(self, officeId):
        params = self._idataauthfacade.generator_params(officeId=officeId)
        try:
            result = self._idataauthfacade.getDataAuthGroupByCreateOffice(params=params)
