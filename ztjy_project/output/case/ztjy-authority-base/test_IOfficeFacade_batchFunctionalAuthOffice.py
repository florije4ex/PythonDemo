...
def pairs_datas():
    ...
    """
    param_data = [
        ['ignore', None, '', 'OFFICEIDS', 'OF' * 11],  # OFFICEIDS
...
class Test_IOfficeFacade_batchFunctionalAuthOffice(object):
    def setup_class(self):
        self._iofficefacade = IOfficeFacade()

    @pytest.mark.parametrize('officeIds, functionalCode', pairs_datas())
    def test_IOfficeFacade_batchFunctionalAuthOffice(self, officeIds, functionalCode):
        params = self._iofficefacade.generator_params(officeIds=officeIds, functionalCode=functionalCode)
        try:
            result = self._iofficefacade.batchFunctionalAuthOffice(params=params)
