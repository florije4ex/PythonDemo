...
class Test_IOfficeFacade_batchGetOfficeInfo(object):
    def setup_class(self):
        self._iofficefacade = IOfficeFacade()

    @pytest.mark.parametrize('['officeIds']', '')
    def test_IOfficeFacade_batchGetOfficeInfo(self, officeIds):
        params = self._iofficefacade.generator_params(officeIds=officeIds)
        try:
            result = self._iofficefacade.batchGetOfficeInfo(params=params)
