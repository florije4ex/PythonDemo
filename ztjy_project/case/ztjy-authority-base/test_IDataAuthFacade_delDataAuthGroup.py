...
class Test_IDataAuthFacade_delDataAuthGroup(object):
    def setup_class(self):
        self._idataauthfacade = IDataAuthFacade()

    @pytest.mark.parametrize('dataAuthGroupId, userId, userCenterEnum', pairs_datas())
    def test_IDataAuthFacade_delDataAuthGroup(self, dataAuthGroupId, userId, userCenterEnum):
        params = self._idataauthfacade.generator_params(dataAuthGroupId=dataAuthGroupId, userId=userId, userCenterEnum=userCenterEnum)
        try:
            result = self._idataauthfacade.delDataAuthGroup(params=params)
