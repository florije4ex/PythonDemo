...
def pairs_datas():
    ...
    param_data = [
        ...
        ['ignore', None, '', 'ROLEIDS', 'RO' * 11],  # ROLEIDS
...
class Test_IRoleFacade_updateUserOfficeRoles(object):
    def setup_class(self):
        self._irolefacade = IRoleFacade()

    @pytest.mark.parametrize('userId, operateUserId, userType, officeId, roleIds, systemType', pairs_datas())
    def test_IRoleFacade_updateUserOfficeRoles(self, userId, operateUserId, userType, officeId, roleIds, systemType):
        params = self._irolefacade.generator_params(userId=userId, operateUserId=operateUserId, userType=userType, officeId=officeId, roleIds=roleIds, systemType=systemType)
        try:
            result = self._irolefacade.updateUserOfficeRoles(params=params)
