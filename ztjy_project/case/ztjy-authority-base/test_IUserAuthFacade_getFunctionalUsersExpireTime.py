...
class Test_IUserAuthFacade_getFunctionalUsersExpireTime(object):
    def setup_class(self):
        self._iuserauthfacade = IUserAuthFacade()

    @pytest.mark.parametrize('functionalCode, officeId, userIds, userTypeEnum', pairs_datas())
    def test_IUserAuthFacade_getFunctionalUsersExpireTime(self, functionalCode, officeId, userIds, userTypeEnum):
        params = self._iuserauthfacade.generator_params(functionalCode=functionalCode, officeId=officeId, userIds=userIds, userTypeEnum=userTypeEnum)
        try:
            result = self._iuserauthfacade.getFunctionalUsersExpireTime(params=params)
