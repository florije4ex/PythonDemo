...
class Test_IDataAuthFacade_addDataAuthGroupInOffice(object):
    def setup_class(self):
        self._idataauthfacade = IDataAuthFacade()

    @pytest.mark.parametrize('officeId, dataAuthGroupId, userId, userCenterEnum', pairs_datas())
    def test_IDataAuthFacade_addDataAuthGroupInOffice(self, officeId, dataAuthGroupId, userId, userCenterEnum):
        params = self._idataauthfacade.generator_params(officeId=officeId, dataAuthGroupId=dataAuthGroupId, userId=userId, userCenterEnum=userCenterEnum)
        try:
            result = self._idataauthfacade.addDataAuthGroupInOffice(params=params)
