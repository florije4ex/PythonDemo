...
class Test_IFunctionalFacade_delFunctionalPackageByCode(object):
    def setup_class(self):
        self._ifunctionalfacade = IFunctionalFacade()

    @pytest.mark.parametrize('functionalPackageCode, operateUserId, userCenterEnum', pairs_datas())
    def test_IFunctionalFacade_delFunctionalPackageByCode(self, functionalPackageCode, operateUserId, userCenterEnum):
        params = self._ifunctionalfacade.generator_params(functionalPackageCode=functionalPackageCode, operateUserId=operateUserId, userCenterEnum=userCenterEnum)
        try:
            result = self._ifunctionalfacade.delFunctionalPackageByCode(params=params)
