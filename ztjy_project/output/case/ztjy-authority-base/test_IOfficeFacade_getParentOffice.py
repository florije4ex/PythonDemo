...
class Test_IOfficeFacade_getParentOffice(object):
    def setup_class(self):
        self._iofficefacade = IOfficeFacade()

    @pytest.mark.parametrize('['officeId']', '')
    def test_IOfficeFacade_getParentOffice(self, officeId):
        params = self._iofficefacade.generator_params(officeId=officeId)
        try:
            result = self._iofficefacade.getParentOffice(params=params)
