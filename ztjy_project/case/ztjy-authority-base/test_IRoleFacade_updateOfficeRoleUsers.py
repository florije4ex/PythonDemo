...
class Test_IRoleFacade_updateOfficeRoleUsers(object):
    def setup_class(self):
        self._irolefacade = IRoleFacade()

    @pytest.mark.parametrize('userIds, userType, officeId, roleId, operateUserId, systemType', pairs_datas())
    def test_IRoleFacade_updateOfficeRoleUsers(self, userIds, userType, officeId, roleId, operateUserId, systemType):
        params = self._irolefacade.generator_params(userIds=userIds, userType=userType, officeId=officeId, roleId=roleId, operateUserId=operateUserId, systemType=systemType)
        try:
            result = self._irolefacade.updateOfficeRoleUsers(params=params)
