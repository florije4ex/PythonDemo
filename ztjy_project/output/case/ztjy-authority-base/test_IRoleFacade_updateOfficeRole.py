...
class Test_IRoleFacade_updateOfficeRole(object):
    def setup_class(self):
        self._irolefacade = IRoleFacade()

    @pytest.mark.parametrize('roleDTO, userId, userCenterEnum', pairs_datas())
    def test_IRoleFacade_updateOfficeRole(self, roleDTO, userId, userCenterEnum):
        params = self._irolefacade.generator_params(roleDTO=roleDTO, userId=userId, userCenterEnum=userCenterEnum)
        try:
            result = self._irolefacade.updateOfficeRole(params=params)
