...
class Test_IOfficeFuncExpireAuthFacade_getOfficeAuthWithBaseAndTryFunctionals(object):
    def setup_class(self):
        self._iofficefuncexpireauthfacade = IOfficeFuncExpireAuthFacade()

    @pytest.mark.parametrize('officeId, systemTypeEnum', pairs_datas())
    def test_IOfficeFuncExpireAuthFacade_getOfficeAuthWithBaseAndTryFunctionals(self, officeId, systemTypeEnum):
        params = self._iofficefuncexpireauthfacade.generator_params(officeId=officeId, systemTypeEnum=systemTypeEnum)
        try:
            result = self._iofficefuncexpireauthfacade.getOfficeAuthWithBaseAndTryFunctionals(params=params)
