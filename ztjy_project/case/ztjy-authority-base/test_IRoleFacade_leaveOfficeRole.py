...
class Test_IRoleFacade_leaveOfficeRole(object):
    def setup_class(self):
        self._irolefacade = IRoleFacade()

    @pytest.mark.parametrize('userId, operateUserId, userType, officeId, roleId, systemType', pairs_datas())
    def test_IRoleFacade_leaveOfficeRole(self, userId, operateUserId, userType, officeId, roleId, systemType):
        params = self._irolefacade.generator_params(userId=userId, operateUserId=operateUserId, userType=userType, officeId=officeId, roleId=roleId, systemType=systemType)
        try:
            result = self._irolefacade.leaveOfficeRole(params=params)
