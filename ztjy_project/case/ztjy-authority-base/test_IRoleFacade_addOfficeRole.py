...
def pairs_datas():
    ...
    """
    param_data = [
        ['ignore', None, '', 'ROLEDTO', 'RO' * 11],  # ROLEDTO
...
class Test_IRoleFacade_addOfficeRole(object):
    def setup_class(self):
        self._irolefacade = IRoleFacade()

    @pytest.mark.parametrize('roleDTO, operateUserId, userCenterEnum', pairs_datas())
    def test_IRoleFacade_addOfficeRole(self, roleDTO, operateUserId, userCenterEnum):
        params = self._irolefacade.generator_params(roleDTO=roleDTO, operateUserId=operateUserId, userCenterEnum=userCenterEnum)
        try:
            result = self._irolefacade.addOfficeRole(params=params)
