...
class Test_IOfficeFacade_updateOffice(object):
    def setup_class(self):
        self._iofficefacade = IOfficeFacade()

    @pytest.mark.parametrize('officeId, officeName, userId, userCenterEnum', pairs_datas())
    def test_IOfficeFacade_updateOffice(self, officeId, officeName, userId, userCenterEnum):
        params = self._iofficefacade.generator_params(officeId=officeId, officeName=officeName, userId=userId, userCenterEnum=userCenterEnum)
        try:
            result = self._iofficefacade.updateOffice(params=params)
