...
class Test_IFunctionalFacade_delOfficeFunctionalPackageAuth(object):
    def setup_class(self):
        self._ifunctionalfacade = IFunctionalFacade()

    @pytest.mark.parametrize('officeId, functionalPackageCode, operateUserId, userCenterEnum', pairs_datas())
    def test_IFunctionalFacade_delOfficeFunctionalPackageAuth(self, officeId, functionalPackageCode, operateUserId, userCenterEnum):
        params = self._ifunctionalfacade.generator_params(officeId=officeId, functionalPackageCode=functionalPackageCode, operateUserId=operateUserId, userCenterEnum=userCenterEnum)
        try:
            result = self._ifunctionalfacade.delOfficeFunctionalPackageAuth(params=params)
