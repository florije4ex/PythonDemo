...
class Test_IFunctionalFacade_getOfficeAuthFunctionalPackages(object):
    def setup_class(self):
        self._ifunctionalfacade = IFunctionalFacade()

    @pytest.mark.parametrize('['officeId']', '')
    def test_IFunctionalFacade_getOfficeAuthFunctionalPackages(self, officeId):
        params = self._ifunctionalfacade.generator_params(officeId=officeId)
        try:
            result = self._ifunctionalfacade.getOfficeAuthFunctionalPackages(params=params)
