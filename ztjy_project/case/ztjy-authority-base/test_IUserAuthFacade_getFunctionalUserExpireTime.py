...
class Test_IUserAuthFacade_getFunctionalUserExpireTime(object):
    def setup_class(self):
        self._iuserauthfacade = IUserAuthFacade()

    @pytest.mark.parametrize('functionalCode, userId, officeId, userTypeEnum', pairs_datas())
    def test_IUserAuthFacade_getFunctionalUserExpireTime(self, functionalCode, userId, officeId, userTypeEnum):
        params = self._iuserauthfacade.generator_params(functionalCode=functionalCode, userId=userId, officeId=officeId, userTypeEnum=userTypeEnum)
        try:
            result = self._iuserauthfacade.getFunctionalUserExpireTime(params=params)
