...
class Test_IFunctionalFacade_getRoleAuthFunctionals(object):
    def setup_class(self):
        self._ifunctionalfacade = IFunctionalFacade()

    @pytest.mark.parametrize('['roleId']', '')
    def test_IFunctionalFacade_getRoleAuthFunctionals(self, roleId):
        params = self._ifunctionalfacade.generator_params(roleId=roleId)
        try:
            result = self._ifunctionalfacade.getRoleAuthFunctionals(params=params)
