...
class RPCCreater(object):
    ...
    def __init__(self, filter_keyword = None):
        ...
        self.root_path = 'ztjy_project/output/'
    ...
    def create_rpc_file(self, rpc_data: str = None):
        ...
        method_list.sort()
        # 判断项目目录，若不存在则创建
        if self.check_dir(self.root_path + 'common_projects/' + project_name):
            # 创建文件，并写入数据
            with open(self.root_path + 'common_projects/' + project_name + '/' + file_name + '.py', mode='w+',
    ...
    def add_case_file(self, project_name: str = None, interface_name: str = None, method: str = None,
        ...
        # 检查文件是否已经存在，不存在则创建
        if self.check_dir(self.root_path + 'case/' + project_name):
            with open(self.root_path + 'case/' + project_name + '/' + file_name + '.py', mode='w+',
                      encoding='utf-8') as file_handle:
                # 写入标准文件头
                self.add_case_file_header(file_handle)
                self.add_case_file_datas(file_handle, project_name=project_name.replace('-', '_'),
    ...
    def add_case_file_datas(self, file_handle, project_name: str = None, interface_name: str = None, method: str = None,
        ...
        # 写入用例负责人
        file_handle.write("@allure.description('负责人：XXXXXXX')\n")
        # 写入测试类定义
        file_handle.writelines(
        ...
        if len(params_names) < 2:
            file_handle.write(space(4) + "@pytest.mark.parametrize('{param_name}', '')\n".format(
                    param_name=params_names))
        else:
            file_handle.write(space(4) + "@pytest.mark.parametrize('{param_name}', pairs_datas())\n".format(
                    param_name=', '.join(params_names)))
        file_handle.writelines([space(4) + 'def test_{interface_name}_{method}(self, {param_name}):\n'.format(
                ...
                param_name=', '.join(params_names)),
                                space(8) + 'params = self._{interface_name}.generator_params({params})\n'.format(
                                        ...
                                        params=', '.join([p + '=' + p for p in params_names])),
                                space(8) + 'try:\n',
                                ...
                                space(
                                        12) + 'assert_that(e.__str__()).contains("NullPointerException", "NoSuchMethodException")\n',
                                space(8) + 'else:\n',
                                space(12) + '# TODO 断言逻辑待完善\n',
    ...
    def add_allpaire_fun(self, file_handle, params_dict):
        ...
        file_handle.writelines([
                "def pairs_datas():\n",
                space(4) + '"""\n',
                space(4) + "采取 pairwise ( 结对测试 ) 的测试策略进行用例设计，减少测试组合的同时最大化用例性价比\n",
                space(4) + ":return: 符合正交分析结果的测试数据集\n",
                space(4) + '"""\n',
                space(4) + "param_data = [\n"])
        # 逐个参数写入
        for param_type, param_names in params_dict.items():
        ...
        file_handle.writelines([space(4) + ']\n',
                                space(4) + 'return AllPairs(param_data)\n',
                                '\n\n'])
...
class ParamAnalysiser(object):
    def __init__(self):
        ...
        self.facade_root = 'ztjy_project/rpc_creater/searchcenter/'
    ...
    def get_params(self, project_name: str = None, interface_name: str = None, method_name: str = None):
        ...
        params_dict = {}
        # 解析接口文档
        with open(file=self.facade_root + 'facade/{project_name}/{interface_name}.java'.format(
    ...
    def get_special_type_full_name(self, project_name: str = None, interface_name: str = None, param_type: str = None):
        ...
        with open(file=self.facade_root + 'facade/{project_name}/{interface_name}.java'.format(
            ...
            if len(type_full_name) == 0:
                ...
                # 重新解析接口文档
                with open(
                        file=self.facade_root + 'facade/{project_name}/{interface_name}.java'.format(
                                project_name=project_name,
                                interface_name=interface_name.split('.')[-1]),
